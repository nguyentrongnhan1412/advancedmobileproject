...
class CourseService {
  ...
  static Future<Map<String, dynamic>> getListCourseWithPagination({required String token, required int page, required int size, required String search,}) async {
    final response = await get(
      Uri.parse('$baseUrl/course?page=$page&size=$size${search.isNotEmpty
          ? '&q=$search'
          : ''}'),
      headers: {
  ...
  static Future<Course> getCourseDetailById({required String token, required String courseId}) async {
    final response = await get(
